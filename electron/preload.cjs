...
contextBridge.exposeInMainWorld('studioHost', {
  ...
  getTrendScoutStatus: () => ipcRenderer.invoke('studio:get-trend-scout-status'),
  runTrendScout: (input) => ipcRenderer.invoke('studio:run-trend-scout', input),
  createTrendAdaptation: (input) => ipcRenderer.invoke('studio:create-trend-adaptation', input),
